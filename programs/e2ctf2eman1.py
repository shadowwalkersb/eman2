		...
		for i in range( pssize ) :
			...
			for j in range( 1, len( empsf ) ) :
				if empsf[j][0] >= s and empsf[j-1][0] < s :
					sf.append( old_div(( ( empsf[j][0] - s ) * empsf[j-1][1] + ( s - empsf[j-1][0] ) * empsf[j][1] ), ( empsf[j][0] - empsf[j-1][0] )) )
					notfound = False
					break
			if notfound :
		...
		while ctf.defocus < parm_max :
			...
			for i in range( i0, i1 ) :
				if ctfmodel[i-1] >= ctfmodel[i] and ctfmodel[i+1] > ctfmodel[i] :
					...
					zeronum += 1.
			if zeronum > 0. :
				defocusvals.append( [ old_div(zerosum, zeronum), ctf.defocus, firstzero, secondzero ] )
...
def getamp( ps, sf, e1ctf, e2ctf, ds ) :
	...
	for i in range( 3, 7 ) :
		...
		ampval = sqrt( max( old_div(( ps[i] - e1bg ), max( sf[i] * ctf[i] ** 2, 0.0001 )), 0. ) )
		if ampval > 0. and ampval < 20. :
			amplist.append( ampval )
	if len( amplist ) > 2 :
		amplist.sort( )
		return old_div(sum( amplist[1:-1] ), len( amplist[1:-1] ))
	elif len( amplist ) > 0 :
		return old_div(sum( amplist[:] ), len( amplist[:] ))
	else :
		return 0.
...
def addsnr( intsnr, ps, bg ) :
	...
	# Calculate SNR
	snr = [ old_div(( ps[i] - bg[i] ), max( bg[i], 0.0001 )) for i in range( len( ps ) ) ]
...
def smooth( seq, kernel ) :
	...
	if ( len( kernel ) - 1 ) % 2 :
		kernel.insert( 0, 0. )
	maxj = int( old_div(( len( kernel ) - 1 ), 2) )
