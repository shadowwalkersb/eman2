...
def main():
	...
	for inp_num, infile in enumerate(args, start=1):
		...
		if infile[0] == ":": 	# special flag to create a new image
			num_inp_images = 2
		elif EMUtil.get_image_count(infile) == 1 and gimme_image_dimensions3D(infile)[2] != 1:
				num_inp_images = 1
		elif not (EMUtil.get_image_count(infile) == 1 and gimme_image_dimensions3D(infile)[2] != 1):
				num_inp_images = 2
