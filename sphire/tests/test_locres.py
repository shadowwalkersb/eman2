...
from os import path
from .test_module import ABSOLUTE_OLDBIN_PATH,ABSOLUTE_PATH_TO_SPHIRE_DEMO_RESULTS_FOLDER_NEW,ABSOLUTE_BIN_PATH,remove_dir,IMAGE.3D
import unittest
...
class Test_helperFunction(unittest.TestCase):
    def test_makeAngRes(self):
        return_new= fu.makeAngRes(freqvol=IMAGE.3D, nx=IMAGE_3D.get_xsize(), ny=IMAGE_3D.get_ysize(), nz=IMAGE_3D.get_zsize(), pxSize=1.14, freq_to_real=True)
        return_old = oldfu.makeAngRes(freqvol=IMAGE.3D, nx=IMAGE_3D.get_xsize(), ny=IMAGE_3D.get_ysize(),nz=IMAGE_3D.get_zsize(), pxSize=1.14, freq_to_real=True)
