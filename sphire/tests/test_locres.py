...
class Test_helperFunction(unittest.TestCase):
    def test_makeAngRes(self):
        return_new= fu.makeAngRes(freqvol=IMAGE.3D, nx=IMAGE.3D.get_xsize(), ny=IMAGE_3D.get_ysize(), nz=IMAGE_3D.get_zsize(), pxSize=1.14, freq_to_real=True)
        return_old = oldfu.makeAngRes(freqvol=IMAGE.3D, nx=IMAGE.3D.get_xsize(), ny=IMAGE_3D.get_ysize(),nz=IMAGE_3D.get_zsize(), pxSize=1.14, freq_to_real=True)
