...
from os import path
from sphire.tests.test_module import ABSOLUTE_OLDBIN_PATH,ABSOLUTE_PATH_TO_SPHIRE_DEMO_RESULTS_FOLDER_NEW,ABSOLUTE_BIN_PATH,remove_dir,IMAGE.2D,get_arg_from_pickle_file,ABSOLUTE_PATH, give_ali_vol_data, give_ormq_data
import unittest
...
class Test_helperFunctions(unittest.TestCase):
    ...
    def test_create_mask(self):
        return_new=fu.create_mask(size=IMAGE.2D.get_2dview().shape, index_particle_pixels=numpy.where(IMAGE.2D.get_2dview() > 0.1), fraction_size=0.25)
        return_old = oldfu.create_mask(size=IMAGE.2D.get_2dview().shape,
                                    index_particle_pixels=numpy.where(IMAGE.2D.get_2dview() > 0.1), fraction_size=0.25)
        self.assertTrue(array_equal(return_new.get_2dview(), return_old.get_2dview()))
