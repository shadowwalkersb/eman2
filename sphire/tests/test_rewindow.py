...
from .test_module import IMAGE.2D,IMAGE_3D
...
TOLERANCE=0.00001   #tolerance value for the unitests
...
class Test_helperFunctions(unittest.TestCase):
    ...
    def test_mrk_resample2d_subrate1(self):
        return_new = fu.mrk_resample2d(img=deepcopy(IMAGE.2D), sub_rate=1, target_size = None)
        return_old = oldfu.mrk_resample2d(img=deepcopy(IMAGE.2D), sub_rate=1, target_size = None)
    ...
    def test_mrk_resample2d_subrate_negative_RuntimeError(self):
        with self.assertRaises(RuntimeError) as cm_new:
            fu.mrk_resample2d(img=deepcopy(IMAGE.2D), sub_rate=-0.9, target_size = None)
        with self.assertRaises(RuntimeError) as cm_old:
            oldfu.mrk_resample2d(img=deepcopy(IMAGE.2D), sub_rate=-0.9, target_size = None)
        msg = str(cm_new.exception).split("'")
        msg_old = str(cm_old.exception).split("'")
        self.assertEqual(msg[0].split(" ")[0], "InvalidValueException")
        self.assertEqual(msg[3], "x size <= 0")
