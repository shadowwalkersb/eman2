...
class Test_generate_random_averages(unittest.TestCase):
    def test_null_K(self):
        return_old = oldfu.generate_random_averages(data=[IMAGE.2D,IMAGE.2D,IMAGE_2D], K=0, rand_seed=-1)
        return_new = fu.generate_random_averages(data=[IMAGE.2D,IMAGE.2D,IMAGE_2D], K=0, rand_seed=-1)
        self.assertTrue(array_equal(return_old,return_new))
        self.assertTrue(array_equal([], return_new))

    def test(self):
        return_old = oldfu.generate_random_averages(data=[IMAGE.2D,IMAGE.2D,IMAGE_2D], K=2, rand_seed=-1)
        return_new = fu.generate_random_averages(data=[IMAGE.2D,IMAGE.2D,IMAGE_2D], K=2, rand_seed=-1)
    ...
    def test_K_higher_lenData_returns_IndexError_list_index_out_of_range(self):
        with self.assertRaises(IndexError) as cm_new:
            fu.generate_random_averages(data=[IMAGE.2D,IMAGE.2D,IMAGE_2D], K=5, rand_seed=-1)
        with self.assertRaises(IndexError) as cm_old:
            oldfu.generate_random_averages(data=[IMAGE.2D,IMAGE.2D,IMAGE_2D], K=5, rand_seed=-1)
