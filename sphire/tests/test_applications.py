...
ABSOLUTE_PATH_TO_STACK = "bdb:" + path.join(
    ABSOLUTE_PATH_TO_SPHIRE_DEMO_RESULTS_FOLDER, "Substack/isac_substack"
)
...
class Test_ali2d_MPI(unittest.TestCase):
    argum = get_arg_from_pickle_file(
        path.join(ABSOLUTE_PATH, "pickle files/applications.ali2d_base")
    )
...
class Test_cpy(unittest.TestCase):
    ...
    def test_error_hdf_not_found(self):
        ins_list = path.join(
            ABSOLUTE_PATH_TO_SPHIRE_DEMO_RESULTS_FOLDER, "not_found.hdf"
        )
...
class Test_project3d(unittest.TestCase):
    ...
    def test_save_on_hdf(self):
        ...
        self.assertTrue(numpy.allclose(return_new[2].get_3dview(), return_old[2].get_3dview() , atol= 0.1))
...
class Test_ali_vol(unittest.TestCase):
    argum = get_arg_from_pickle_file(
        path.join(ABSOLUTE_PATH, "pickle files/applications.ali_vol")
    )
    ...
    def test_NoneRadius(self):
        ...
        self.assertTrue(allclose(return_new.get_3dview(), return_old.get_3dview(), atol = 1))
        self.assertTrue(
            array_equal(
                return_new.get_3dview().flatten()[60100:60250],
                [
...
class Test_header(unittest.TestCase):
    ...
    data = get_arg_from_pickle_file(
        path.join(ABSOLUTE_PATH, "pickle files/applications.header")
    )[0]
...
class Test_within_group_refinement(unittest.TestCase):
    argum = get_arg_from_pickle_file(
        path.join(ABSOLUTE_PATH, "pickle files/applications.within_group_refinement")
    )[0]
    randomize = False
