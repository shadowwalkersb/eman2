...
def iter_isac(stack, ir, ou, rs, xr, yr, ts, maxit, CTF, snr, dst, FL, FH, FF, init_iter, main_iter, iter_reali, \
	...
	if myid == main_node:
		sxprint("****************************************************************************************************")
		sxprint("*                                                                                                  *")
		sxprint("*                 Beginning of the ISAC program                "+strftime("%a, %d %b %Y %H:%M:%S", localtime())+"           *")
		sxprint("*                                                                                                  *")
		sxprint("* Iterative Stable Alignment and Clustering                                                        *")
		...
		sxprint("*                                                                                                  *")
		sxprint("* Last updated: 07/23/2015 PAP                                                                     *")
		sxprint("****************************************************************************************************")
		sxprint("*                                       Generation %3d                                             *"%(generation))
	...
	if  not candidatesexist:
		if myid == main_node:
			sxprint("******************************************************************************************")
			sxprint("*            Beginning of the first phase           "+strftime("%a, %d %b %Y %H:%M:%S", localtime())+"            *")
			sxprint("*                                                                                        *")
			sxprint("* The first phase is an exploratory phase. In this phase, we set the criteria very       *")
			sxprint("* loose and try to find as many candidate class averages as possible. This phase         *")
			sxprint("* typically should have 10 to 20 rounds (default = 20). The candidate class averages are *")
			sxprint("* stored in class_averages_candidate_generation_n.hdf.                                   *")
			sxprint("******************************************************************************************")
		...
		while Iter <= max_round and couldnt_find_stable < 3 and K > 3:
			if myid == main_node: 
				sxprint("################################################################################")
				sxprint("#           Beginning of Round %2d           "%Iter+strftime("%a, %d %b %Y %H:%M:%S", localtime())+"          #")
				sxprint("################################################################################")
				sxprint("     Initialization of averages using EQ-mref")
				sxprint("********************************************************************************")
			...
			if myid == main_node:
				sxprint("**********************************************************************")
				sxprint("     Processing of candidate averages   "+strftime("%a, %d %b %Y %H:%M:%S", localtime()))
				sxprint("**********************************************************************")
	
			for mloop in range(1, match_first+1):
				if myid == main_node:
					sxprint("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
					sxprint("     Loop %3d for 2-way matching   "%mloop+strftime("%a, %d %b %Y %H:%M:%S", localtime()))
					sxprint("~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~")
			...
			if myid == main_node:
				...
				mem_len = len(members)
				sxprint("In Round #%d, we found %d stable and reproducible averages, accounted for %d particles.  "%(Iter, len(matched_data), mem_len))
			else:
				mem_len = 0
				members = []
		...
		if myid == main_node:
			...
			sxprint("******************************************************************************************")
			sxprint("*              End of the first phase             "+strftime("%a, %d %b %Y %H:%M:%S", localtime())+"              *")
			sxprint("******************************************************************************************")
		return
	...
	if myid == main_node:
		sxprint("")
		sxprint("******************************************************************************************")
		sxprint("*           Beginning of the second phase         "+strftime("%a, %d %b %Y %H:%M:%S", localtime())+"             *")
		sxprint("*                                                                                        *")
		sxprint("* The second phase is where the actual class averages are generated, it typically has    *")
		sxprint("* 3~9 iterations (default = 5) of matching. The first half of iterations are 2-way       *")
		sxprint("* matchings, the second half of iterations are 3-way matchings, and the last iteration is*")
		sxprint("* 4-way matching. In the second phase, three files will be generated:                    *")
		sxprint("* class_averages_generation_n.hdf : class averages generated in this generation          *")
		sxprint("* generation_n_accounted.txt      : IDs of accounted particles in this generation        *")
		sxprint("* generation_n_unaccounted.txt    : IDs of unaccounted particles in this generation      *")
		sxprint("******************************************************************************************")
		try:
			refim = EMData.read_images(refim_stack)
			sxprint("* Using existing %4d candidate class averages                                            *"%len(refim))
		except:
			refim = []
		nrefim = len(refim)
	else:
	...
	for mloop in range(1, match_second+1):
		...
		if myid == main_node:		
			sxprint("################################################################################")
			sxprint("#       Iteration %2d for %d-way matching       "%(mloop, wayness)+strftime("%a, %d %b %Y %H:%M:%S", localtime())+"        #")
			sxprint("################################################################################")
		...
		if K_left > 0:
			if myid == main_node: 
				sxprint("**********************************************************************")
				sxprint("        Generating initial averages for unaccounted for images        ")
				sxprint("**********************************************************************")
				sxprint("   Number of images unaccounted for = %d     Number of groups = %d"%(nleft, K_left))
...
#					refim[i].write_image("init_group%d_2nd_phase_round%d.hdf"%(color, mloop), i)
		...
		if myid == main_node:
			sxprint("**********************************************************************")
			sxprint("     Run the main part of ISAC program   "+strftime("%a, %d %b %Y %H:%M:%S", localtime()))
			sxprint("**********************************************************************")
			sxprint("    Number of images = %d               Number of groups = %d"%(ndata, K))
...
#				refim[i].write_image("log_mainPart_" + str(color) + "_" + str(mloop) + ".hdf", i)
		...
		if mloop != match_second:
			if myid == main_node:
				sxprint("**********************************************************************")
				sxprint("     Run the %d-way matching algorithm  "%wayness+strftime("%a, %d %b %Y %H:%M:%S", localtime()))
				sxprint("**********************************************************************")
				...
				if mloop == two_way_loop:
					...
					if len(refim_all) > K:
						sxprint("Since the number of unique 2-way matches is larger than the number of groups (%d), we only use the first %d of them."%(K, K))
						refim_all = refim_all[:K]
					elif len(refim_all) < K:
						defi = K - len(refim_all)
						sxprint("Since the number of unique 2-way matches is smaller than the number of groups (%d), we have to append %d random images."%(K, defi))
						for i in range(defi):
...
#				refim[i].write_image("log_afterMatching_" + str(color) + "_" + str(mloop) + ".hdf", i)
	...
	if myid == main_node:
		sxprint("**********************************************************************")
		sxprint("       Run the final %d-way matching algorithm  "%indep_run+strftime("%a, %d %b %Y %H:%M:%S", localtime()))
		sxprint("**********************************************************************")
	...
	if myid == main_node:
		...
		for i in range(l_STB):
			...
			sxprint("Group %d ...... Mirror consistent rate = %f"%(i, mirror_consistent_rate))
			sxprint("Group %d ...... Average pixel error = %f"%(i, pix_err))
			sxprint("Group %d ...... Size of stable subset = %d"%(i, l_stable_members))
			sxprint("Group %d ......"%i, end=' ')

			if l_stable_members <= thld_grp:
				sxprint("Size of stable subset smaller than the threshold, discarded\n")
				continue
			sxprint("Size of stable subset larger than the threshold, kept\n")
		...
		sxprint("In the second phase, we found %d stable and reproducible averages that account for %d particles.  "%(ave_num, len(members_acc)))
		...
		sxprint("******************************************************************************************")
		sxprint("*     End of the second phase             "+strftime("%a, %d %b %Y %H:%M:%S", localtime())+"            *")
		sxprint("******************************************************************************************")
	else:
		for i in range(l_STB):
			node_to_run = i%(number_of_proc-1)+1
	...
	if myid == main_node:
		sxprint("****************************************************************************************************")
		sxprint("*                                                                                                  *")
		sxprint("*                   End of the ISAC program                 "+strftime("%a, %d %b %Y %H:%M:%S", localtime())+"            *")
		sxprint("*                                                                                                  *")
		sxprint("****************************************************************************************************")
	return
...
def match_independent_runs(data, refi, n_group, T):
	...
	sxprint("%d-way match: total cost of matches over threshold: "%(len(Parts)), sum(cost_by_match_thresh))
	sxprint("%d-way match: total number of matches over threshold: "%(len(Parts)), len(cost_by_match_thresh))
	sxprint("%d-way match: cost by match over threshold: "%(len(Parts)), cost_by_match_thresh)
	sxprint(" ")
...
def match_2_way(data, refi, indep_run, thld_grp, FH, FF, find_unique=True, wayness=2, suffix=""):
	...
	if find_unique:
		...
		sxprint("Found %d unique class averages through %d-way matching"%(len(reproducible_avgs_unique), wayness))
		return reproducible_avgs_unique
	else:
		sxprint("Found %d class averages through %d-way matching"%(len(reproducible_avgs), wayness))
		...
		for j in range((i/size+1)*size, size_all):
			...
			if float(len(m)) > m_th*(len(m1)+len(m2))/2:
				if flag[i] == 0 and flag[j] == 0:
					...
					sxprint("Impossible: Something is wrong!")
