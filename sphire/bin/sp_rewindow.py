...
def main():
	...
	usage = program_name + """  input_micrograph_pattern  input_rebox_pattern  output_directory  --selection_list=SELECTION_TEXT_FILE  --box_size=BOX_SIZE  --skip_invert  --mic_resample_ratio=RATIO  --swap_ctf_params=CTER_PARTRES_FILE_PATH  --check_consistency
...
"""
	...
	i_enum = -1
	i_enum += 1; idx_rebox_mic_coord_id       = i_enum  # Original box rebox ID
	i_enum += 1; idx_rebox_mic_coord_x        = i_enum  # Original box center x-coordinate
	i_enum += 1; idx_rebox_mic_coord_y        = i_enum  # Original box center y-coordinate
	i_enum += 1; idx_rebox_mic_resample_ratio = i_enum  # Resample ratio used previous windowing or rewindowing
	i_enum += 1; idx_rebox_ctf_defocus        = i_enum  # Defocus of CTF
	i_enum += 1; idx_rebox_ctf_cs             = i_enum  # CS of CTF
	i_enum += 1; idx_rebox_ctf_voltage        = i_enum  # Voltage of CTF
	i_enum += 1; idx_rebox_ctf_apix           = i_enum  # Pixel size [A] of CTF
	i_enum += 1; idx_rebox_ctf_bfactor        = i_enum  # B-factor of CTF
	i_enum += 1; idx_rebox_ctf_ampcont        = i_enum  # Amplitude contrast of CTF
	i_enum += 1; idx_rebox_ctf_dfdiff         = i_enum  # Defocus difference or astigmatism amplitude of CTF
	i_enum += 1; idx_rebox_ctf_dfang          = i_enum  # Defocus angle or astigmatism angle of CTF
	i_enum += 1; idx_rebox_proj_phi           = i_enum  # Eulerian angle for 3D reconstruction (azimuthal) of projection parameters (xform.projection)
	i_enum += 1; idx_rebox_proj_theta         = i_enum  # Eulerian angle for 3D reconstruction (tilt) of projection parameters (xform.projection)
	i_enum += 1; idx_rebox_proj_psi           = i_enum  # Eulerian angle for 3D reconstruction (in-plane rotation of projection) of projection parameters (xform.projection)
	i_enum += 1; idx_rebox_proj_sx            = i_enum  # Shift in x direction of projection parameters (xform.projection)
	i_enum += 1; idx_rebox_proj_sy            = i_enum  # Shift in y direction of projection parameters (xform.projection)
	i_enum += 1; idx_rebox_pp_def_error_accum = i_enum  # Accumulated per-particle defocus error
	i_enum += 1; idx_rebox_pp_mag_error_accum = i_enum  # Accumulated per-particle magnification error
	i_enum += 1; idx_rebox_chunk_id           = i_enum  # NOT SUPPORTED YET (Toshio Moriya 2018/07/10)
	i_enum += 1; n_idx_rebox                  = i_enum

	# For indices of CTF parameters
	i_enum = -1
	i_enum += 1; idx_ctf_defocus = i_enum  # Defocus of CTF
	i_enum += 1; idx_ctf_cs      = i_enum  # CS of CTF
	i_enum += 1; idx_ctf_voltage = i_enum  # Voltage of CTF
	i_enum += 1; idx_ctf_apix    = i_enum  # Pixel size [A] of CTF
	i_enum += 1; idx_ctf_bfactor = i_enum  # B-factor of CTF
	i_enum += 1; idx_ctf_ampcont = i_enum  # Amplitude contrast of CTF
	i_enum += 1; idx_ctf_dfdiff  = i_enum  # Defocus difference or astigmatism amplitude of CTF
	i_enum += 1; idx_ctf_dfang   = i_enum  # Defocus angle or astigmatism angle of CTF
	i_enum += 1; n_idx_ctf       = i_enum
	assert(n_idx_ctf == 8) # Due to the error handling in generate_ctf(), the argument has to be a list with length of 6 or 8.
	
	# For indices of projection parameters
	i_enum = -1
	i_enum += 1; idx_proj_phi   = i_enum  # Eulerian angle for 3D reconstruction (azimuthal) of projection parameters (xform.projection)
	i_enum += 1; idx_proj_theta = i_enum  # Eulerian angle for 3D reconstruction (tilt) of projection parameters (xform.projection)
	i_enum += 1; idx_proj_psi   = i_enum  # Eulerian angle for 3D reconstruction (in-plane rotation of projection) of projection parameters (xform.projection)
	i_enum += 1; idx_proj_sx    = i_enum  # Shift in x direction of projection parameters (xform.projection)
	i_enum += 1; idx_proj_sy    = i_enum  # Shift in y direction of projection parameters (xform.projection)
	i_enum += 1; n_idx_proj     = i_enum
	...
	while my_mpi_proc_id == main_mpi_proc:
		...
		# --------------------------------------------------------------------------------
		sxprint(" ")
		sxprint("Checking the rebox files...")
		rebox_path_list = glob.glob(rebox_pattern)
		
		# Check error condition of rebox file path list
		sxprint("Found %d rebox files in %s directory." % (len(rebox_path_list), os.path.dirname(rebox_pattern)))
		...
		for rebox_path in rebox_path_list:
			...
			rebox_id_substr = rebox_path[rebox_id_substr_head_idx:rebox_id_substr_tail_idx]
		...
		if ctf_params_src is not None:
			if use_real_ctf_params:
				# This should be string for CTER partres (CTF parameter) file
				sxprint(" ")
				sxprint("The program will use the CTF parameters stored in the CTER partres file specified through -swap_ctf_params option, while ignoring the CTF parameters in the input rebox parameters file...")
				sxprint("  It will also ignores all defocus errors extracted from the input rebox paramters file and reset it to zero...")
				sxprint(" ")
				...
				# This should be string for CTER partres (CTF parameter) file
				sxprint(" ")
				sxprint("The program will use the simulated ideal CTF with the pixel size %f specified through -swap_ctf_params option, while ignoring the CTF parameters in the input rebox parameters file..." % (float(ctf_params_src)))
				sxprint("  It will also ignores all defocus errors extracted from the input rebox paramters file and reset it to zero...")
		...
		for mic_id_substr in global_entry_dict:
			...
			if subkey_selected_mic_basename in mic_id_entry: 
				...
				if not subkey_rebox_path in mic_id_entry:
					...
					warinnig_messages.append("    associated rebox file %s." % (rebox_path))
		...
		if options.check_consistency:
			...
			for mic_id_substr in global_entry_dict:
				...
				if not subkey_rebox_path in mic_id_entry:
					rebox_path = rebox_pattern.replace("*", mic_id_substr)
					consistency_messages.append("    associated rebox file %s." % (rebox_path))
		...
		sxprint("Rejected by no rebox file : %6d" % (len(no_rebox_mic_id_substr_list)))
		if ctf_params_src is not None:
			if use_real_ctf_params:
	...
	for mic_id_substr_idx, mic_id_substr in enumerate(valid_mic_id_substr_list):
		...
		if (len(rebox_list) == 0):
			sxprint("For %s, the associate rebox file %s does not contain any entries. Skipping..." % (mic_basename, rebox_path))
			n_mic_reject_no_rebox_entry += 1
			continue
		assert (len(rebox_list) > 0)
		if (len(rebox_list[0]) != n_idx_rebox):
			sxprint("For %s, the format of the associate rebox file %s is not valid. It should contain %d columns. Skipping..." % (mic_basename, rebox_path, n_idx_rebox))
		...
		for rebox_id in range(len(rebox_list)):
			...
			rebox_resample_ratio = float(rebox_list[rebox_id][idx_rebox_mic_resample_ratio]) # accumulated resample ratio including per-particle magnification adjustment runs, starting from the original micrograph resample ratio used in the original sxwindow run.
			
			ctf_entry = [0.0]*n_idx_ctf
			if ctf_params_src is None:
				# Get particle CTF parameters
				ctf_entry[idx_ctf_defocus] = float(rebox_list[rebox_id][idx_rebox_ctf_defocus])
				ctf_entry[idx_ctf_cs]      = float(rebox_list[rebox_id][idx_rebox_ctf_cs])
				ctf_entry[idx_ctf_voltage] = float(rebox_list[rebox_id][idx_rebox_ctf_voltage])
				ctf_entry[idx_ctf_apix]    = float(rebox_list[rebox_id][idx_rebox_ctf_apix])
				ctf_entry[idx_ctf_bfactor] = float(rebox_list[rebox_id][idx_rebox_ctf_bfactor])
				ctf_entry[idx_ctf_ampcont] = float(rebox_list[rebox_id][idx_rebox_ctf_ampcont])
				ctf_entry[idx_ctf_dfdiff]  = float(rebox_list[rebox_id][idx_rebox_ctf_dfdiff])
				ctf_entry[idx_ctf_dfang]   = float(rebox_list[rebox_id][idx_rebox_ctf_dfang])
			else:
			...
			proj_entry[idx_proj_theta] = float(rebox_list[rebox_id][idx_rebox_proj_theta])
			...
			pp_def_error_accum = float(rebox_list[rebox_id][idx_rebox_pp_def_error_accum])
			pp_mag_error_accum = float(rebox_list[rebox_id][idx_rebox_pp_mag_error_accum])
			...
			if rebox_resample_ratio < 1.0:
				...
				if my_mpi_proc_id == main_mpi_proc:
					sxprint("MRK_DEBUG: ")
					sxprint("MRK_DEBUG: BEFORE applying rebox_resample_ratio := {}".format(rebox_resample_ratio))
					sxprint("MRK_DEBUG: ctf_obj.apix := {}, proj_entry[idx_proj_sx] := {}, proj_entry[idx_proj_sy] := {}".format(ctf_obj.apix, proj_entry[idx_proj_sx], proj_entry[idx_proj_sy]))
				ctf_obj.apix *= rebox_resample_ratio
				proj_entry[idx_proj_sx] /= rebox_resample_ratio
				proj_entry[idx_proj_sy] /= rebox_resample_ratio
				if my_mpi_proc_id == main_mpi_proc:
					sxprint("MRK_DEBUG: AFTER applying rebox_resample_ratio := {}".format(rebox_resample_ratio))
					sxprint("MRK_DEBUG: ctf_obj.apix := {}, proj_entry[idx_proj_sx] := {}, proj_entry[idx_proj_sy] := {}".format(ctf_obj.apix, proj_entry[idx_proj_sx], proj_entry[idx_proj_sy]))
			else:
			...
			if mic_resample_ratio < 1.0:
				...
				if my_mpi_proc_id == main_mpi_proc:
					sxprint("MRK_DEBUG: ")
					sxprint("MRK_DEBUG: BEFORE applying mic_resample_ratio := {}".format(mic_resample_ratio))
					sxprint("MRK_DEBUG: ctf_obj.apix := {}, proj_entry[idx_proj_sx] := {}, proj_entry[idx_proj_sy] := {}".format(ctf_obj.apix, proj_entry[idx_proj_sx], proj_entry[idx_proj_sy]))
				...
				if my_mpi_proc_id == main_mpi_proc:
					sxprint("MRK_DEBUG: AFTER applying mic_resample_ratio := {}".format(mic_resample_ratio))
					sxprint("MRK_DEBUG: ctf_obj.apix := {}, proj_entry[idx_proj_sx] := {}, proj_entry[idx_proj_sy] := {}".format(ctf_obj.apix, proj_entry[idx_proj_sx], proj_entry[idx_proj_sy]))
				if my_mpi_proc_id == main_mpi_proc:
					sxprint("Resample micrograph to pixel size %6.4f [A/Pixels] from %6.4f [A/Pixels] and window segments from resampled micrograph." % (ctf_obj.apix, src_pixel_size))
			else:
			...
			if( (0 <= x - resampled_box_half) and ( x + resampled_box_half < nx ) and (0 <= y - resampled_box_half) and ( y + resampled_box_half < ny ) ):
				try:
					...
					sxprint("MRK_DEBUG: ")
					sxprint("MRK_DEBUG: mic_resample_ratio := {}, box_size := {}, box_half := {}".format(mic_resample_ratio, box_size, box_half))
					sxprint("MRK_DEBUG: pp_mag_error_accum := {}, resampled_box_size := {}, resampled_box_half := {}".format(pp_mag_error_accum, resampled_box_size, resampled_box_half))
					sxprint("MRK_DEBUG: nx := {}, ny := {}".format(nx, ny))
					sxprint("MRK_DEBUG: x := {}, y := {}, x0 := {}, y0 := {}, x-x0 := {}, y-y0 := {}".format(x, y, x0, y0, x-x0, y-y0))
					raise
			else:
				rebox_reject_out_of_boundary_messages.append("rebox ID = %04d: x = %4d, y = %4d, box_size = %4d " % (rebox_id, x, y, box_size))
				# sxprint("MRK_DEBUG: rebox_reject_out_of_boundary_messages[-1] := %s" % rebox_reject_out_of_boundary_messages[-1])
				continue
			
			if pp_mag_error_accum != 1.0:
				assert (pp_mag_error_accum > 0.0)
				particle_img = mrk_resample2d(particle_img, pp_mag_error_accum, box_size)
				### #
			...
			if pp_def_error_accum != 0.0:
				if my_mpi_proc_id == main_mpi_proc:
					sxprint("MRK_DEBUG: ")
					sxprint("MRK_DEBUG: BEFORE applying pp_def_error_accum := {}".format(pp_def_error_accum))
					sxprint("MRK_DEBUG: ctf_obj.defocus := {}".format(ctf_obj.defocus))
				ctf_obj.defocus += pp_def_error_accum
				if my_mpi_proc_id == main_mpi_proc:
					sxprint("MRK_DEBUG: AFTER applying pp_def_error_accum := {}".format(pp_def_error_accum))
					sxprint("MRK_DEBUG: ctf_obj.defocus := {}".format(ctf_obj.defocus))
			...
			try:
				...
				sxprint("The standard deviation of the particle image associated with %dth rebox entry in micrograph %s for %s is zero unexpectedly. Skipping..." % (rebox_id, mic_path, mic_basename))
				continue
			...
			particle_img.set_attr("ptcl_source_coord", [int(rebox_list[rebox_id][idx_rebox_mic_coord_x]), int(rebox_list[rebox_id][idx_rebox_mic_coord_y])])
			particle_img.set_attr("ptcl_source_coord_id", rebox_id)
			particle_img.set_attr('data_n', rebox_id)  # NOTE: Toshio Moriya 2017/11/20: same as ptcl_source_coord_id but the other program uses this header entry key...
			### #
			### # NOTE: Toshio Moriya 2018/05/17
			### # Since the pp_mag_error_accum will essentially brings back to all particles to the same pixel size 
			### # (i.e. micrograph pixel size or micrograph-level global pixel size).
		...
		if len(rebox_reject_out_of_boundary_messages) > 0:
			...
			if RUNNING_UNDER_MPI:
				...
				assert os.path.exists(reject_out_of_boundary_dir), "MRK_DEBUG"
			coords_reject_out_of_boundary_path = os.path.join(reject_out_of_boundary_dir, os.path.splitext(os.path.basename(rebox_path))[0] + "_reject_out_of_boundary.txt")
			...
			for coords_reject_out_of_boundary_message in rebox_reject_out_of_boundary_messages:
		...
		n_global_rebox_reject_out_of_boundary += len(rebox_reject_out_of_boundary_messages)
	...
	if main_mpi_proc == my_mpi_proc_id:
		...
		sxprint("Rejected by no rebox entries       : %6d" % (n_mic_reject_no_rebox_entry))
		sxprint("Rejected by invalid rebox format   : %6d" % (n_mic_reject_invalid_rebox_format))
		sxprint(" ")
		sxprint("Global summary of rebox level processing...")
		sxprint("Detected                           : %6d" % (n_global_rebox_detect))
		sxprint("Processed                          : %6d" % (n_global_rebox_process))
		sxprint("Rejected by out of boundary        : %6d" % (n_global_rebox_reject_out_of_boundary))
		if n_global_rebox_reject_out_of_boundary > 0:
			assert os.path.exists(reject_out_of_boundary_dir), "MRK_DEBUG"
			sxprint("    NOTE: Information of rejected rebox by out of boundary are saved in %s files." % (os.path.join(reject_out_of_boundary_dir, os.path.splitext(os.path.basename(rebox_pattern))[0] + "_reject_out_of_boundary.txt" )))
		else:
