...
masters_from_groups_vs_everything_else_comm = mpi_comm_split(MPI_COMM_WORLD,
                                                             Blockdata["main_node"] == Blockdata["myid_on_node"],
                                                             Blockdata["myid_on_node"])
Blockdata["color"], Blockdata["no_of_groups"], balanced_processor_load_on_nodes = get_colors_and_subsets(
    Blockdata["main_node"], MPI_COMM_WORLD, Blockdata["myid"], \
...
if (Blockdata["no_of_groups"] > 1):
    Blockdata["main_shared_nodes"] = [Blockdata["node_volume"][0] * Blockdata["no_of_processes_per_group"],
...
def create_subgroup():
    ...
    Blockdata["nodes"] = [Blockdata["node_volume"][0] * Blockdata["ncpuspernode"],
...
def depth_clustering(work_dir, depth_order, initial_id_file, params, previous_params, log_main):
    ...
    for depth in range(depth_order):  # layers, depth_order = 1 means one layer and two boxes.
        ...
        if (Blockdata["myid"] == Blockdata["main_node"]):
            log_main.add(
                '                        Layer %d has %d pairs of independent sorting runs' % (depth, n_cluster_boxes))
            log_main.add(' ')
            log_main.add(' ')
        ...
        if keepchecking == 0:
            ...
            for nbox in range(n_cluster_boxes):
                ...
                if checkingbox == 0:
                    bad_box = depth_clustering_box(nbox_dir, input_accounted_file, input_unaccounted_file, params,
                                                   previous_params, nbox, log_main)
            ...
            if bad_clustering != 1:
                ...
                for nbox in range(0, n_cluster_boxes, 2):
                    ...
                    minimum_grp_size, maximum_grp_size, accounted_list, unaccounted_list, bad_clustering, stop_generation, stat_list = \
                        do_boxes_two_way_comparison_mpi(nbox, input_box_parti1, input_box_parti2, depth_order - depth,
                                                        log_main)
    ...
    if input_list2 is not None:  # Track 2
        ...
        for indep in range(2):
            tmp_assignment = swap_accounted_with_unaccounted_elements_mpi(os.path.join(box_dir, "previous_NACC.txt"), \
                                                                          ...
                                                                          log_file, number_of_groups, swap_ratio)
        ...
        if number_of_groups < 2: ERROR("number_of_groups should be at least larger than 1", "depth_box_initialization",
                                       1, Blockdata["myid"])

        new_assignment = create_nrandom_lists_from_given_pids(box_dir, os.path.join(box_dir, \
                                                                                    "previous_all_indexes.txt"),
                                                              number_of_groups, 2)
...
def output_iter_results(box_dir, ncluster, NACC, NUACC, minimum_grp_size, list_of_stable, unaccounted_list, img_per_grp,
                        log_main):
    ### single node
...
def output_clusters(output_dir, partition, unaccounted_list, not_include_unaccounted, log_main):
    ...
    for ic in range(len(nclasses)):
        if len(nclasses[ic]) >= max(
                Tracker["constants"]["img_per_grp"] // 2 ** max(Tracker["constants"]["img_per_grp_split_rate"], 1) \
...
def do_analysis_on_identified_clusters(clusters, log_main):
    ...
    if ss is not None: tmpres1, tmpres2 = do_one_way_anova_scipy(clusters, ss, name_of_variable="smearing",
                                                                 log_main=log_main)
    if norms:          tmpres1, tmpres2 = do_one_way_anova_scipy(clusters, norms, name_of_variable="norm",
                                                                 log_main=log_main)
    return
...
def depth_clustering_box(work_dir, input_accounted_file, input_unaccounted_file, params, previous_params, nbox,
                         log_main):
    global Tracker, Blockdata
    from sp_utilities import read_text_file, wrap_mpi_bcast, write_text_file, write_text_row
    ...
    while (keepgoing == 1):
        ...
        if Tracker["nosmearing"]:
            ...
            parameterstructure = read_paramstructure_for_sorting(iter_id_init_file, paramstructure_dict,
                                                                 paramstructure_dir)
        mpi_barrier(MPI_COMM_WORLD)
        ...
        if nruns == 0:  # only do it in the first box
            ...
            Tracker["nxinit"], Tracker["freq_fsc143_cutoff"] = get_sorting_image_size(original_data, iter_id_init_file,
                                                                                      current_number_of_groups, \
                                                                                      parameterstructure,
                                                                                      norm_per_particle, log_main)
        ...
        cdata, rdata, fdata = downsize_data_for_sorting(original_data, preshift=True, npad=1,
                                                        norms=norm_per_particle)  # pay attentions to shifts!
        del original_data
        mpi_barrier(MPI_COMM_WORLD)
        srdata = precalculate_shifted_data_for_recons3D(rdata, parameterstructure, Tracker["refang"], \
                                                        Tracker["rshifts"], Tracker["delta"], Tracker["avgnorm"],
                                                        Tracker["nxinit"], \
                                                        Tracker["constants"]["nnxo"], Tracker["nosmearing"],
        ...
        while ((iter < box_niter) and (converged == 0)):
            for indep_run_iter in range(2):
                ...
                tmp_final_list, premature = Kmeans_minimum_group_size_orien_groups(cdata, fdata, srdata, \
                                                                                   MGSKmeans_index_file, params,
                                                                                   minimum_grp_size, clean_volumes=True)
            ...
            if Blockdata["myid"] == Blockdata["main_node"]:
                minimum_grp_size1, maximum_grp_size1, list_of_stable, unaccounted_list, iter_current_iter_ratio, selected_number_of_groups, info_table = \
                    do_withinbox_two_way_comparison(iter_dir, nbox, nruns,
                                                    iter)  # two partitions are written in partition_dir as partition_%03d.txt
            else:
                unaccounted_list = 0
                list_of_stable = 0
                minimum_grp_size1 = 0
            ...
            if Tracker["constants"]["do_swap_au"]:
                ...
                swap_ratio = 0.0
            if Blockdata["myid"] == Blockdata["main_node"]:
                if abs(iter_current_iter_ratio - iter_previous_iter_ratio < 1.0) and iter_current_iter_ratio > 90.:
                    converged = 1
            else:
                converged = 0
            ...
            if (converged == 0) and (iter < box_niter):
                ...
                for indep in range(2):
                    tmp_list = swap_accounted_with_unaccounted_elements_mpi(accounted_file, unaccounted_file, \
                                                                            log_main, current_number_of_groups,
                                                                            swap_ratio)
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        if ncluster > 0:
            if (ncluster == 1):
                ...
                log_main.add('In independent run  %d, the program found %d groups' % (nbox, ncluster))
            bad_clustering = 0
        else:
...
def check_mpi_settings(log_main):
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        ...
        for ifreq in range(len(fsc_refinement)): fsc_refinement[ifreq] = fsc_refinement[ifreq] * q / (
                    1. - fsc_refinement[ifreq] * (1. - q))
        res = 0.0
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        ...
        log_main.add(
            '----------------------------------------------------------------------------------------------------------------')
        log_main.add("Number of processes: %d number of nodes:  %d.  Number of processes per node:  %d." % (
        Blockdata["nproc"], Blockdata["no_of_groups"], Blockdata["no_of_processes_per_group"]))
    try:
        ...
        raw_data_size_per_node = float(
            Tracker["constants"]["total_stack"] * image_org_size * image_org_size) * 4.0 / 1.e9 / Blockdata[
                                     "no_of_groups"]
    ...
    if current_mpi_settings_is_bad == 1:   ERROR("Initial info is not provided", "check_mpi_settings", 1,
                                                 Blockdata["myid"])
    try:
    ...
    if (
            total_memory - sys_required_mem - raw_data_size_per_node - volume_size_per_node - sorting_data_size_per_node - 5.0) < 0.0:
        current_mpi_settings_is_bad = 1
        new_nproc = raw_data_size * (2. * ratio ** 2 + 1.) * Blockdata["no_of_processes_per_group"] / (
                    total_memory - 5. - sys_required_mem - volume_size_per_node)
    ...
    if (images_per_cpu < 5): ERROR("Number of images per processor is less than 5",
...
def get_sorting_image_size(original_data, partids, number_of_groups, sparamstructure, snorm_per_particle, log_main):
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]): write_text_file(avg_fsc, os.path.join(Tracker["directory"],
                                                                                            "fsc_image_size.txt"))
    del iter_assignment
    del proc_list
    del fsc_data
...
def compute_noise(image_size):
    ...
    if Tracker["applybckgnoise"]:  # from SPARX refinement only
        ...
        for i in range(len(Blockdata["bckgnoise"])): Blockdata["unrolldata"].append(
            Util.unroll1dpw(ny, ny, Blockdata["bckgnoise"][i]))
...
def get_params_for_analysis(orgstack, ali3d_params, smearing_file, smearing_number):
    ...
    if smearing_file is not None:
        try:
            smearing_list = read_text_file(smearing_file)
        except:
            smearing_list = None
    else:
        smearing_list = None
...
def do_one_way_anova_scipy(clusters, value_list, name_of_variable="variable", log_main=None):
    ...
    if K == 2:
        ...
        res = stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12)
    elif K == 14:
        res = stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13)
    elif K == 15:
        res = stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14)
    elif K == 16:
        res = stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15)
    elif K == 17:
        res = stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16)
    elif K == 18:
        res = stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17)
    elif K == 19:
        res = stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, x18)
    elif K == 20:
        res = stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, x18, x19)
    elif K == 21:
        res = stats.f_oneway(x0, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15, x16, x17, x18, x19,
                             x20)
    elif K == 22:
    ...
    from math import sqrt
    for i in range(K):
        log_main.add('{:5} {:^7d} {:^8d} {:12.4f} {:12.4f}'.format('ANOVA', i, len(replicas[i]), res_table_stat[i][0],
                                                                   sqrt(res_table_stat[i][1])))
    log_main.add(' ')
    log_main.add('ANOVA  Pair-wise tests')
    log_main.add('{:5} {:^3} {:^3} {:^12} {:^12} {:^12} {:^12}'.format('ANOVA', 'A', 'B', 'avgA', 'avgB', 'F_value',
                                                                       'Significance'))
    for ires in range(K - 1):
        for jres in range(ires + 1, K):
            cres = stats.f_oneway(replicas[ires], replicas[jres])
            log_main.add(
                '{:5} {:^3d} {:^3d} {:12.4f} {:12.4f} {:12.3f} {:12.4f} '.format('ANOVA', ires, jres, avgs[ires],
                                                                                 avgs[jres], cres[0], cres[1] * 100.))
    log_main.add(' ')
    log_main.add(
        '================================================================================================================\n')
    return res[0], res[1]
...
def check_3dmask(log_main):
    ...
    try:
        ...
        Tracker["fuse_freq"] = int(
            Tracker["constants"]["pixel_size"] * Tracker["constants"]["nnxo"] / Tracker["constants"]["fuse_freq"] + 0.5)
    Tracker = wrap_mpi_bcast(Tracker, Blockdata["main_node"], MPI_COMM_WORLD)
...
def AI_MGSKmeans(iter_assignment, last_iter_assignment, best_assignment, keepgoing, best_score, stopercnt,
                 minimum_grp_size, log_file):
    # single cpu function
    import collections
...
def Kmeans_minimum_group_size_orien_groups(cdata, fdata, srdata, \
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        ...
        if len(lpartids) == 1:  # Not true for sorting
            iter_assignment = []
            for im in range(len(lpartids[0])): iter_assignment.append(
                randint(0, number_of_groups - 1))  # simple version
        else:
            iter_assignment = lpartids[0]
    else:
        iter_assignment = 0
    ...
    minimum_group_size_ratio = min((minimum_group_size * Tracker["number_of_groups"]) / float(Tracker["total_stack"]),
                                   0.95)
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        ...
        msg = "Total_stack:  %d K = : %d  nxinit: %d  CTF:  %s  Symmetry:  %s  stop percentage: %f  3-D mask: %s focus mask: %s  Comparison method: %s  minimum_group_size: %d orien  %d" % \
              (Tracker["total_stack"], Tracker["number_of_groups"], Tracker["nxinit"], Tracker["constants"]["CTF"],
               ...
               stopercnt, Tracker["constants"]["mask3D"], Tracker["constants"]["focus3D"],
               Tracker["constants"]["comparison_method"], minimum_group_size, len(ptls_in_orien_groups))
        log_main.add(msg)
    ...
    while total_iter < max_iter:
        ...
        for iref in range(number_of_groups):
            if (Blockdata["myid"] == Blockdata["last_node"]):
                ...
                if (Tracker["nxinit"] != nnn): ref_vol = fdecimate(ref_vol, Tracker["nxinit"], Tracker["nxinit"],
                                                                   Tracker["nxinit"], True, False)
        ...
        if Blockdata["myid"] == Blockdata["main_node"]:
            ...
            best_score, changed_nptls, keepgoing, best_assignmen, iter_assignment = \
                AI_MGSKmeans(iter_assignment, last_iter_assignment, best_assignment, keepgoing, best_score, stopercnt,
                             minimum_group_size, log_main)
    ...
    if require_check_setting:
        if (Blockdata["myid"] == Blockdata["main_node"]): print(
            "Warning: the sorting settings, such as img_per_grp requires a check")
    return partition, premature


def do_assignment_by_dmatrix_orien_group_minimum_group_size(dmatrix, orien_group_members, number_of_groups,
                                                            minimum_group_size_ratio):
    import numpy as np
    ...
    while len(rmatrix[0]) > nima - minimum_group_size * number_of_groups:
        ...
        if len(value_list) < number_of_groups:
            for i in range(len(index_list)):
                if tarray[index_list[i]] == tarray[duplicate_list[0]]: duplicate_list.append(
                    index_list[i])  # find all duplicated ones
            ...
            for i in range(len(duplicate_list)):  # swap the first row with the next row
                index_column = 1
                while rmatrix[duplicate_list[i]][
                    index_column] in value_list: index_column += 1  # search along column non-equal ones
                value_list.append(rmatrix[duplicate_list[i]][index_column])
                rmatrix[duplicate_list[i]][0], rmatrix[duplicate_list[i]][index_column] = rmatrix[duplicate_list[i]][
                                                                                              index_column], \
    ...
    kmeans_ptl_list = (
        np.delete(np.array(list(range(nima))), np.array(results).ravel())).tolist()  # ravel works only for even size
    del rmatrix
    for iptl in range(len(kmeans_ptl_list)):
        max_indexes = np.argwhere(submatrix[:, kmeans_ptl_list[iptl]] <= submatrix[:, kmeans_ptl_list[iptl]][
        ...
        try:
            phi, theta, psi, sx, sy, chunk_id, particle_group_id = partstack[lpartids[im]][0], partstack[lpartids[im]][
                1], \
                                                                   ...
                                                                   partstack[lpartids[im]][5], partstack[lpartids[im]][
                                                                       6]
        except:
            phi, theta, psi, sx, sy, chunk_id, particle_group_id = partstack[lpartids[im]][0], partstack[lpartids[im]][
                1], \
...
def get_data_prep_compare_rec3d(partids, partstack, return_real=False, preshift=True, npad=1):
    ...
    if Tracker["constants"]["focus3D"]:  # focus mask is applied
        if Blockdata["myid"] == Blockdata["main_node"]:
            ...
            focus3d = model_blank(Tracker["constants"]["nnxo"], Tracker["constants"]["nnxo"],
    ...
    for im in range(nima):
        ...
        if Tracker["applybckgnoise"]:
            ...
            if Tracker["constants"]["comparison_method"] == "cross": Util.mulclreal(cdata[im], Blockdata["unrolldata"][
                particle_group_id])
        if Tracker["constants"]["focus3D"]:
...
def get_shrink_data_final(nxinit, procid, original_data=None, oldparams=None, \
    ...
    if Blockdata["bckgnoise"]:
        ...
        for i in range(len(Blockdata["bckgnoise"])):
            temp = [0.0] * nnx
            for k in range(nnx):
                if (Blockdata["bckgnoise"][i].get_value_at(k) > 0.0):  temp[k] = 1.0 / sqrt(
...
def read_data_for_sorting(partids, partstack, previous_partstack):
    ...
    partstack = wrap_mpi_bcast(partstack, Blockdata["main_node"])
    if (Blockdata["myid"] == Blockdata["main_node"]):
        previous_partstack = read_text_row(previous_partstack)
    else:
...
def read_paramstructure_for_sorting(partids, paramstructure_dict_file, paramstructure_dir):
    ...
    from sp_applications import MPI_start_end
    if (Blockdata["myid"] == Blockdata["main_node"]):
        lcore = read_text_file(partids, -1)
    else:
        lcore = 0
...
def copy_oldparamstructure_from_meridien_MPI(selected_iteration, log_main):
    global Tracker, Blockdata
    from sp_utilities import read_text_row, write_text_row, read_text_file, wrap_mpi_bcast
    from sp_applications import MPI_start_end
    import json
    ...
    old_refinement_iter_directory = os.path.join(Tracker["constants"]["refinement_dir"],
                                                 "main%03d" % selected_iteration)
    old_refinement_previous_iter_directory = os.path.join(Tracker["constants"]["refinement_dir"],
                                                          "main%03d" % (selected_iteration - 1))
    if (Blockdata["myid"] == Blockdata["main_node"]):
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]): write_text_row(Tracker["refang"],
                                                                     os.path.join(Tracker["directory"], "refang.txt"))
    Tracker["rshifts"] = read_text_row(os.path.join(old_refinement_iter_directory, "rshifts.txt"))
    if (Blockdata["myid"] == Blockdata["main_node"]): write_text_row(Tracker["refang"],
                                                                     os.path.join(Tracker["directory"], "rshifts.txt"))
    my_last_params = read_text_file(
        os.path.join(old_refinement_previous_iter_directory, "params_%03d.txt" % (selected_iteration - 1)), -1)
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        ...
        write_text_file(my_parstack,
                        os.path.join(Tracker["constants"]["masterdir"], "previous_refinement_parameters.txt"))
    ...
    if Blockdata["myid"] == Blockdata["main_node"]:
        while os.path.exists(os.path.join(old_refinement_iter_dir, "oldparamstructure",
    ...
    for procid in range(2):
        smearing_list = []
        if (Blockdata["myid"] == Blockdata["main_node"]):
            lcore = read_text_file(os.path.join(Tracker["constants"]["masterdir"], "chunk_%d.txt" % procid))
        else:
            lcore = 0
        ...
        for iproc_index_old in range(istart_old_proc_id, iend_old_proc_id + 1):
            fout = open(os.path.join(Tracker["constants"]["refinement_dir"], "main%03d" % selected_iteration,
            ...
            while (mlocal_id_on_old < len(oldparamstructure_on_old_cpu)) and (ptl_on_this_cpu < im_end):
                ...
                local_dict[local_lcore[nptl_total]] = [Blockdata["myid"], procid, selected_iteration, nptl_total,
                                                       ptl_on_this_cpu]
                ptl_on_this_cpu += 1
                mlocal_id_on_old += 1
                nptl_total += 1
    ...
    for procid in range(2):
        ...
        local_smearing_list = []
        for im in range(len(oldparamstructure[procid])): local_smearing_list.append(
            len(oldparamstructure[procid][im][2]))
...
### 8
def precalculate_shifted_data_for_recons3D(prjlist, paramstructure, refang, rshifts, delta, avgnorms, nxinit, nnxo,
                                           nosmearing, norm_per_particle=None, upweighted=False, nsmear=-1):
    ...
    for im in range(len(prjlist)):
        ...
        if nosmearing:
            ...
            prjlist[im].set_attr_dict(
                {"padffted": 1, "is_fftpad": 1, "is_fftodd": 0, "is_complex_ri": 1, "is_complex": 1})
            ...
            for ii in range(len(tdir)):
                ...
                recdata.set_attr_dict({"padffted": 1, "is_fftpad": 1, "is_fftodd": 0, "is_complex_ri": 1,
                                       "is_complex": 1})  # preset already
                if not upweighted: recdata = filt_table(recdata, bckgn)
                recdata.set_attr_dict({"bckgnoise": bckgn, "ctf": ct})
                ipsi = tdir[ii] % 100000
                iang = tdir[ii] / 100000
                set_params_proj(recdata, [refang[iang][0], refang[iang][1], refang[iang][2] + ipsi * delta, 0.0, 0.0],
...
def downsize_data_for_rec3D(original_data, particle_size, return_real=False, npad=1):
    ...
    for im in range(nima):
        ...
        if Tracker["nosmearing"]:
            image = fshift(image, s2x, s2y)
        else:
            image = cyclic_shift(image, int(round(sx)), int(round(sy)))
        st = Util.infomask(image, mask2D, False)
        image -= st[0]
        image /= st[1]
        image = fft(image)
...
def compare_two_images_eucd(data, ref_vol, fdata):
    ...
    for im in range(len(data)):
        ...
        if Tracker["applybckgnoise"]:
            peaks[im] = -Util.sqed(data[im], rtemp, ctfs[im],
...
def compare_two_images_cross(data, ref_vol):
    global Tracker, Blockdata
    from sp_utilities import same_ctf
    ny = data[0].get_ysize()
    peaks = len(data) * [None]
    ...
    for im in range(len(data)):
        ...
        if data[im].get_attr("is_complex") == 1: data[im].set_attr("is_complex", 0)
        if Tracker["constants"]["focus3D"]:
            peaks[im] = Util.innerproduct(ref, data[im], None) / nrmref
        else:
            if Tracker["applybckgnoise"]:
...
def refilling_global_scheme_mpi(clusters, unaccounted_list, number_of_clusters, log_file, swap_ratio):
    ...
    for i in range(len(out_clusters)): out_clusters[i].sort()
    return out_clusters
...
def swap_accounted_with_unaccounted_elements_mpi(accounted_file, unaccounted_file, log_file, number_of_groups,
                                                 swap_ratio):
    global Tracker, Blockdata
    import random
    import copy
...
def patch_to_do_k_means_match_clusters_asg_new(ptp1, ptp2):
    ...
    if len(ptp1) != len(ptp2):
        for i in range(len(ptp1)): ptp1[i] = array(ptp1[i], "int32")
        for i in range(len(ptp2)):    ptp2[i] = array(ptp2[i], "int32")
        alist = []
        blist = []
        for a in ptp1:
...
def do_boxes_two_way_comparison_mpi(nbox, input_box_parti1, input_box_parti2, depth, log_main):
    ...
    if Blockdata["myid"] == Blockdata["main_node"]:
        ...
        if (len(core1) != len(core2)):  ERROR("Two partitions have different lengths", "do_boxes_two_way_comparison", 1,
                                              0)
        ...
        log_main.add(
            '{:>5} {:>8}  {:^8}   {:>15} {:>22}  {:>5}'.format('Group', '    size', ' status ', 'reproducibility',
                                                               'random reproducibility', ' std '))
        from math import sqrt
        for index_of_any in range(len(list_stable)):
            ...
            score3 = float((np.intersect1d(ptp1[newindeces[index_of_any][0]], ptp2[newindeces[index_of_any][1]])).size) \
                     / float(
                (np.union1d(ptp1[newindeces[index_of_any][0]], ptp2[newindeces[index_of_any][1]])).size) * 100.
            if (score3 > gave[index_of_any] + Tracker["constants"]["random_group_elimination_threshold"] * gvar[
                index_of_any]):
                ...
                new_list.append(any)
                nclass += 1
                log_main.add('{:>5} {:>8d}   {:^8}      {:>7.1f}      {:>15.1f}           {:>5.1f}'.format(index_of_any,
                                                                                                           len(any),
                                                                                                           'accepted',
                                                                                                           score3, \
                                                                                                           gave[
    ...
    if nclass == 0:
        ...
        if depth > 1:
            if Blockdata["myid"] == Blockdata["main_node"]:
                log_main.add(
                    'There are no clusters larger than the user provided minimum group size %d.' % Tracker["constants"][
                        "minimum_grp_size"])
                ...
                newindeces, list_stable, nb_tot_objs, patch_elements = patch_to_do_k_means_match_clusters_asg_new(ptp1,
                                                                                                                  ptp2)
        ...
        if Blockdata["myid"] == Blockdata["main_node"]:
            ...
            box1_dir = os.path.join(Tracker["constants"]["masterdir"],
                                    "generation_%03d" % Tracker["current_generation"], "layer%d" % Tracker["depth"],
                                    "nbox%d" % nbox)
            box2_dir = os.path.join(Tracker["constants"]["masterdir"],
                                    "generation_%03d" % Tracker["current_generation"], "layer%d" % Tracker["depth"],
                                    "nbox%d" % (nbox + 1))
        ...
        if Blockdata["myid"] == Blockdata["main_node"]:
            ...
            b = set(accounted_list)
            unaccounted_list = sorted(list(a.difference(b)))
            log_main.add(' {} {} {} {}'.format('  The number of accounted for images:', len(accounted_list),
                                               '  The number of unaccounted for images:', len(unaccounted_list)))
            log_main.add('  The current minimum group size: %d and the maximum group size: %d' % (
            ...
            if depth <= 1:  # the last layer
                ...
                box2_dir = os.path.join(Tracker["constants"]["masterdir"],
                                        "generation_%03d" % Tracker["current_generation"], "layer%d" % Tracker["depth"],
                                        "nbox%d" % (nbox + 1))
                gendir = os.path.join(Tracker["constants"]["masterdir"],
...
def do_withinbox_two_way_comparison(partition_dir, nbox, nrun, niter):
    ...
    for im in range(len(ptp2)): msg += '{:8d} '.format(len(ptp2[im]))
    log_list.append(msg)
    if (len(core1) != len(core2)): ERROR("The two partitions have different lengths", "do_withinbox_two_way_comparison",
                                         1, 0)
    full_list = []
    for a in core1: full_list.append(a[1])
    full_list.sort()
    ...
    log_list.append('{:>8} {:>10} {:>17} {:>8} {:>15}'.format('Group ID', '   size', 'min random size', ' status ',
                                                              'reproducibility'))
    ...
    log_list.append('  The number of accounted for images: %d.  The number of unaccounted for images: %d.' % (
    len(accounted_list), len(unaccounted_list)))
    log_list.append('  The current minimum group size: %d and the maximum group size: %d.' % (
    minimum_group_size, maximum_group_size))
    log_list.append(
        '----------------------------------------------------------------------------------------------------------------')
    return minimum_group_size, maximum_group_size, selected_clusters, unaccounted_list, ratio_accounted, len(
        list_stable), log_list
...
def merge_classes_into_partition_list(classes_list):
    ...
    if len(classes_list) > 0:
        ...
        for index_of_particle in range(len(data_list)): new_index.append(
...
def get_sorting_attr_stack(data_in_core):
    ...
    attr_value_list = []
    for idat in range(len(data_in_core)): attr_value_list.append(
        [data_in_core[idat].get_attr("group"), get_params_proj(data_in_core[idat], xform="xform.projection")])
...
def get_orien_assignment_mpi(angle_step, partids, params, log_main):
    global Tracker, Blockdata
    from sp_applications import MPI_start_end
    from sp_utilities import wrap_mpi_recv, wrap_mpi_bcast, wrap_mpi_send, read_text_row, read_text_file, getvec
    ...
    for iorien in range(len(refa_vecs)):
        if iorien % Blockdata["nproc"] != Blockdata["main_node"]:
            if iorien % Blockdata["nproc"] == Blockdata["myid"]: wrap_mpi_send(ptls_in_orien_groups[iorien],
                                                                               Blockdata["main_node"], MPI_COMM_WORLD)
            if Blockdata["myid"] == Blockdata["main_node"]: ptls_in_orien_groups[iorien] = wrap_mpi_recv(
                iorien % Blockdata["nproc"], MPI_COMM_WORLD)
        mpi_barrier(MPI_COMM_WORLD)
    mpi_barrier(MPI_COMM_WORLD)
...
def get_angle_step_and_orien_groups_mpi(params_in, partids_in, angstep):
    global Tracker, Blockdata
    from sp_applications import MPI_start_end
    from sp_utilities import wrap_mpi_recv, wrap_mpi_bcast, wrap_mpi_send, bcast_number_to_all
    from sp_utilities import read_text_row, read_text_file
...
def steptwo_mpi(tvol, tweight, treg, cfsc=None, regularized=True, color=0):
    ...
    n_iter = 10
    ifi = mpi_iterefa(vol_data.__array_interface__['data'][0], we_data.__array_interface__['data'][0], nx, ny, nz,
                      maxr2, \
                      Tracker["constants"]["nnxo"], Blockdata["myid_on_node"], color,
                      Blockdata["no_of_processes_per_group"], Blockdata["shared_comm"], n_iter)
    if (Blockdata["myid_on_node"] == 0):
        ...
        tvol = fft(tvol)
        tvol = cyclic_shift(tvol, Tracker["constants"]["nnxo"], Tracker["constants"]["nnxo"],
                            Tracker["constants"]["nnxo"])
        tvol = Util.window(tvol, Tracker["constants"]["nnxo"], Tracker["constants"]["nnxo"],
                           Tracker["constants"]["nnxo"])
        tvol.div_sinc(1)
...
def recons3d_4nnsorting_MPI(myid, main_node, prjlist, random_subset, CTF=True, upweighted=True, mpi_comm=None,
    ...
    params = {"size": target_size, "npad": 2, "snr": 1.0, "sign": 1, "symmetry": "c1", "refvol": refvol,
              "fftvol": fftvol, "weight": weight, "do_ctf": do_ctf}
    r = Reconstructors.get("nn4_ctfw", params)
    r.setup()
    # if norm_per_particle == None: norm_per_particle = len(prjlist)*[1.0]
    for im in range(len(prjlist)):
        phi, theta, psi, s2x, s2y = get_params_proj(prjlist[im],
                                                    xform="xform.projection")  # shifts are already applied
        if random_subset == 2:
            bckgn = target_size * [1.]
...
def do3d_sorting(procid, data, myid, mpi_comm=-1):
    ...
    tvol, tweight, trol = recons3d_4nnsorting_MPI(myid=Blockdata["myid"], main_node=Blockdata["nodes"][procid],
                                                  prjlist=data, \
                                                  random_subset=procid, CTF=Tracker["constants"]["CTF"],
        ...
        for index_of_groups in range(Tracker["number_of_groups"]):
            for procid in range(2, 3):
                tvol, tweight, trol = recons3d_4nnsorting_group_MPI(myid=Blockdata["myid"],
                                                                    ...
                                                                    prjlist=data, random_subset=procid,
    ...
    if Blockdata["no_of_groups"] > 1:
        ...
        for iloop in range(nbig_loop):
            for im in range(len(big_loop_colors[iloop])):
                ...
                if (Blockdata["myid"] == Blockdata["last_node"]):
                    ...
                    tweight2 = get_im(
                        os.path.join(Tracker["directory"], "tempdir", "tweight_0_%d.hdf") % index_of_group)
            ...
            for im in range(len(big_loop_colors[iloop])):
                ...
                if (Blockdata["color"] == index_of_colors) and (Blockdata["myid_on_node"] == 0):
                    ...
                    tvol2.write_image(os.path.join(Tracker["directory"], "vol_unfiltered_0_grp%03d_iter%03d.hdf" % (
            ...
            for im in range(len(big_loop_colors[iloop])):
                ...
                if (Blockdata["myid"] == Blockdata["last_node"]):
                    ...
                    tweight2 = get_im(
                        os.path.join(Tracker["directory"], "tempdir", "tweight_1_%d.hdf") % index_of_group)
            ...
            for im in range(len(big_loop_colors[iloop])):
                ...
                if (Blockdata["color"] == index_of_colors) and (Blockdata["myid_on_node"] == 0):
                    ...
                    tag = 7007
                    tvol2 = recv_EMData(sub_main_node_list[index_of_colors], tag, MPI_COMM_WORLD)
                    tvol2.write_image(os.path.join(Tracker["directory"], "vol_unfiltered_1_grp%03d_iter%03d.hdf" % (
                    index_of_group, iteration)))
                    del tvol2
                mpi_barrier(MPI_COMM_WORLD)
            mpi_barrier(MPI_COMM_WORLD)
        ...
        for index_of_group in range(Tracker["number_of_groups"]):
            for iprocid in range(2):
                if (Blockdata["myid"] == Blockdata["last_node"]):
                    tvol2 = get_im(
                        os.path.join(Tracker["directory"], "tempdir", "tvol_%d_%d.hdf") % (iprocid, index_of_group))
                    tweight2 = get_im(
                        os.path.join(Tracker["directory"], "tempdir", "tweight_%d_%d.hdf") % (iprocid, index_of_group))
                    treg2 = get_im(
                        os.path.join(Tracker["directory"], "tempdir", "trol_%d_%d.hdf" % (iprocid, index_of_group)))
                    tag = 7007
                ...
                if (Blockdata["myid"] == Blockdata["main_node"]):
                    # tvol2 = cosinemask(tvol2, radius = Tracker["constants"]["radius"])
                    tvol2.write_image(os.path.join(Tracker["directory"], "vol_unfiltered_%d_grp%03d_iter%03d.hdf" % (
                    iprocid, index_of_group, iteration)))
                mpi_barrier(MPI_COMM_WORLD)
            mpi_barrier(MPI_COMM_WORLD)
    keepgoing = bcast_number_to_all(keepgoing, source_node=Blockdata["main_node"],
                                    mpi_comm=MPI_COMM_WORLD)  # always check
    Tracker = wrap_mpi_bcast(Tracker, Blockdata["main_node"])
    if keepgoing == 0: ERROR("do3d_sorting_groups_trl_iter  %s" % os.path.join(Tracker["directory"], "tempdir"),
...
def get_input_from_sparx_ref3d(log_main):  # case one
    ...
    checking_flag = bcast_number_to_all(checking_flag, Blockdata["main_node"], MPI_COMM_WORLD)
    if checking_flag == 1: ERROR("SPARX refinement directory does not exist", "get_input_from_sparx_ref3d", 1,
                                 Blockdata["myid"])
    if Blockdata["myid"] == Blockdata["main_node"]:
        if Tracker["constants"]["niter_for_sorting"] == -1:  # take the best solution to do sorting
            ...
            while os.path.exists(os.path.join(Tracker["constants"]["refinement_dir"],
                                              "main%03d" % niter_refinement)) and os.path.exists(
                    os.path.join(Tracker["constants"]["refinement_dir"], "main%03d" % niter_refinement,
    ...
    import_from_sparx_refinement = bcast_number_to_all(import_from_sparx_refinement, source_node=Blockdata["main_node"])
    if import_from_sparx_refinement == 0: ERROR("The data stack is not accessible", "get_input_from_sparx_ref3d", 1,
                                                Blockdata["myid"])
    ...
    if Blockdata["myid"] == Blockdata["main_node"]:
        if os.path.exists(os.path.join(Tracker["constants"]["refinement_dir"], "main%03d" % selected_iter,
    ...
    import_from_sparx_refinement = bcast_number_to_all(import_from_sparx_refinement, source_node=Blockdata["main_node"])
    if import_from_sparx_refinement == 0: ERROR("The parameter file of the best solution is not accessible",
    ...
    if Blockdata["myid"] == Blockdata["main_node"]:
        if os.path.exists(os.path.join(Tracker["constants"]["refinement_dir"], "main%03d" % selected_iter, "bckgnoise.hdf")):
            ...
            for search_iter in range(selected_iter - 1, 0, -1):
                if os.path.exists(os.path.join(Tracker["constants"]["refinement_dir"], "main%03d" % search_iter,
                                               "bckgnoise.hdf")):
    ...
    if Blockdata["myid"] == Blockdata["main_node"]:
        ...
        if import_from_sparx_refinement: fsc_curve = read_text_row(
            os.path.join(Tracker["constants"]["masterdir"], "fsc_global.txt"))
        fsc143 = 0
        fsc05 = 0
    ...
    import_from_sparx_refinement = bcast_number_to_all(import_from_sparx_refinement, source_node=Blockdata["main_node"])
    if import_from_sparx_refinement == 0: ERROR("The driver of the best solution is not accessible",
                                                "get_input_from_sparx_ref3d", 1, Blockdata["myid"])
    if Blockdata["myid"] == Blockdata["main_node"]:
    ...
    if import_from_sparx_refinement == 0: ERROR("The index file of the best solution are not accessible",
                                                "get_input_from_sparx_ref3d", 1, Blockdata["myid"])
    if Blockdata["myid"] == Blockdata["main_node"]:
        ...
        if os.path.exists(os.path.join(Tracker["constants"]["refinement_dir"], "main000/particle_groups_1.txt")):
            ...
            import_from_sparx_refinement == 0
    import_from_sparx_refinement = bcast_number_to_all(import_from_sparx_refinement, source_node=Blockdata["main_node"])
    if import_from_sparx_refinement == 0: ERROR("The chunk files and partice group files are not accessible",
                                                "get_input_from_sparx_ref3d", 1, Blockdata["myid"])
    ...
    if Blockdata["myid"] == Blockdata["main_node"]:
        ...
        if Tracker["constants"]["nxinit"] < 0:
            ...
            Tracker["nxinit_refinement"] = Tracker["constants"]["nxinit"]  # User defined window size
        ...
        if not Tracker["constants"]["mask3D"]:
            if Tracker_refinement["constants"]["mask3D"] and (not Tracker["constants"]["do_not_use_3dmask"]):
                refinement_mask3D_path, refinement_mask3D_file = os.path.split(
                    Tracker_refinement["constants"]["mask3D"])  # MRK_DEBUG
    ...
    if update_sym == 1:
        ...
        Tracker["constants"]["orientation_groups"] = max(4, Tracker["constants"]["orientation_groups"] // Blockdata[
            "symclass"].nsym)
...
def get_input_from_datastack(log_main):  # Case three
    ...
    if (Blockdata["myid"] == Blockdata["main_node"]):
        ...
        if (Tracker["nxinit"] > Tracker["constants"]["nnxo"]):
            ERROR("Image size less than minimum permitted $d" % Tracker["nxinit"], "get_input_from_datastack", 1,
                  Blockdata["myid"])
            nnxo = -1
        else:
            if Tracker["constants"]["CTF"]:
    ...
    if (Tracker["constants"]["nnxo"] < 0): ERROR("Image size is negative", "get_input_from_datastack", 1,
                                                 Blockdata["main_node"])
    Tracker["constants"]["pixel_size"] = bcast_number_to_all(Tracker["constants"]["pixel_size"],
                                                             source_node=Blockdata["main_node"])
    if (Tracker["constants"]["radius"] < 1):
        Tracker["constants"]["radius"] = Tracker["constants"]["nnxo"] // 2 - 2
    elif ((2 * Tracker["constants"]["radius"] + 2) > Tracker["constants"]["nnxo"]):
        ERROR("Particle radius set too large!", \
    ...
    if Blockdata["myid"] == Blockdata["main_node"]:
        ...
        for index_of_particle in range(len(xform_proj_list)):
            ...
            xform_proj_list[index_of_particle] = [dp["phi"], dp["theta"], dp["psi"], -dp["tx"], -dp["ty"],
    ...
    for procid in range(2):
        data = get_shrink_data_sorting(os.path.join(Tracker["constants"]["masterdir"], "chunk_%01d.txt" % procid),
    ...
    if (Blockdata["no_of_groups"] == 1):
        ...
        if (Blockdata["myid"] == Blockdata["nodes"][
            1]):  # It has to be 1 to avoid problem with tvol1 not closed on the disk
            tvol0 = get_im(os.path.join(Tracker["directory"], "tempdir", "tvol_0.hdf"))
            tweight0 = get_im(os.path.join(Tracker["directory"], "tempdir", "tweight_0.hdf"))
            tvol1 = get_im(os.path.join(Tracker["directory"], "tempdir", "tvol_1.hdf"))
            tweight1 = get_im(os.path.join(Tracker["directory"], "tempdir", "tweight_1.hdf"))
...
def do3d(procid, data, newparams, refang, rshifts, norm_per_particle, myid, mpi_comm=-1):
    ...
    tvol, tweight, trol = recons3d_trl_struct_MPI(myid=Blockdata["subgroup_myid"], main_node=Blockdata["main_node"],
                                                  prjlist=data, \
...
def do3d_sorting_groups_nofsc_smearing_iter(srdata, partial_rec3d, iteration):
    ...
    for index_of_groups in range(Tracker["number_of_groups"]):
        if partial_rec3d:
            tvol, tweight, trol = recons3d_trl_struct_group_nofsc_shifted_data_partial_MPI(Blockdata["myid"],
                                                                                           Blockdata["last_node"],
                                                                                           Blockdata["nproc"], srdata,
                                                                                           index_of_groups, \
                                                                                           os.path.join(
                                                                                               Tracker["directory"],
                                                                                               "tempdir",
                                                                                               "trol_2_%d.hdf" % index_of_groups), \
                                                                                           os.path.join(
                                                                                               Tracker["directory"],
                                                                                               "tempdir",
                                                                                               "tvol_2_%d.hdf" % index_of_groups), \
                                                                                           os.path.join(
                                                                                               Tracker["directory"],
                                                                                               "tempdir",
    ...
    if Blockdata["no_of_groups"] > 1:
        ...
        for iloop in range(nbig_loop):
            for im in range(len(big_loop_colors[iloop])):
                ...
                if (Blockdata["myid"] == Blockdata["last_node"]):
                    ...
                    tweight2 = get_im(
                        os.path.join(Tracker["directory"], "tempdir", "tweight_2_%d.hdf") % index_of_group)
            ...
            for im in range(len(big_loop_colors[iloop])):
                ...
                try:
                    ...
                    pass
                if Blockdata[
                    "color"] == index_of_colors:  # It has to be 1 to avoid problem with tvol1 not closed on the disk
                    if (Blockdata["myid_on_node"] != 0):
                        tvol2 = model_blank(1)
                        tweight2 = model_blank(1)
                        treg2 = model_blank(1)
                    tvol2 = steptwo_mpi_filter(tvol2, tweight2, treg2, None, Tracker["freq_fsc143_cutoff"], 0.01, False,
                                               color=index_of_colors)  # has to be False!!!
                    del tweight2, treg2
            mpi_barrier(MPI_COMM_WORLD)
            ...
            for im in range(len(big_loop_colors[iloop])):
                ...
                if (Blockdata["color"] == index_of_colors) and (Blockdata["myid_on_node"] == 0):
                    ...
                    tvol2 = recv_EMData(sub_main_node_list[index_of_colors], tag, MPI_COMM_WORLD)
                    tvol2.write_image(
                        os.path.join(Tracker["directory"], "vol_grp%03d_iter%03d.hdf" % (index_of_group, iteration)))
                    del tvol2
            mpi_barrier(MPI_COMM_WORLD)
        mpi_barrier(MPI_COMM_WORLD)
        ...
        for index_of_group in range(Tracker["number_of_groups"]):
            ...
            try:
                ...
                pass
            tvol2 = steptwo_mpi_filter(tvol2, tweight2, treg2, None, Tracker["freq_fsc143_cutoff"], 0.01,
    ...
    keepgoing = bcast_number_to_all(keepgoing, source_node=Blockdata["main_node"],
                                    mpi_comm=MPI_COMM_WORLD)  # always check
    Tracker = wrap_mpi_bcast(Tracker, Blockdata["main_node"])
    if not keepgoing: ERROR("do3d_sorting_groups_trl_iter  %s" % os.path.join(Tracker["directory"], "tempdir"),
                            "do3d_sorting_groups_trl_iter", 1, Blockdata["myid"])
    return


### nofsc insertion #1
def recons3d_trl_struct_group_nofsc_shifted_data_partial_MPI(myid, main_node, nproc, prjlist, group_ID, refvol_file,
    ...
    if CTF:
        ...
        do_ctf = 0
    if not os.path.exists(refvol_file): ERROR("refvol does not exist",
                                              "recons3d_trl_struct_group_nofsc_shifted_data_partial_MPI", 1, myid)
    if not os.path.exists(fftvol_file): ERROR("fftvol does not exist",
                                              "recons3d_trl_struct_group_nofsc_shifted_data_partial_MPI", 1, myid)
    if not os.path.exists(weight_file): ERROR("weight does not exist",
                                              "recons3d_trl_struct_group_nofsc_shifted_data_partial_MPI", 1, myid)

    # refvol
    if myid == main_node:
        target_size = get_im(refvol_file).get_xsize()
    else:
        target_size = 0
        ...
        if nosmearing:
            ...
            if current_group_ID != previous_group_ID:
                if current_group_ID == group_ID:
                    flag = 1.0
                    [phi, theta, psi, s2x, s2y] = get_params_proj(prjlist[im], xform="xform.projection")
                    r.insert_slice(prjlist[im], Transform({"type": "spider", "phi": phi, "theta": theta, "psi": psi}),
                                   flag)
            ...
            if current_group_ID != previous_group_ID:
                if current_group_ID == group_ID:
                    ...
                    for jm in range(len(prjlist[im])):
                        ...
                        r.insert_slice(prjlist[im][jm],
                                       Transform({"type": "spider", "phi": phi, "theta": theta, "psi": psi}),
...
###end of nofsc
def recons3d_trl_struct_group_MPI(myid, main_node, prjlist, random_subset, group_ID, paramstructure,
            ...
            if random_subset == 2:
                if nosmearing:
                    ...
                    for ii in range(len(tdir)):
                        ...
                        ipsi = tdir[ii] % 100000
                        iang = tdir[ii] / 100000
                        r.insert_slice(recdata, Transform(
                            {"type": "spider", "phi": refang[iang][0], "theta": refang[iang][1],
...
def do3d_sorting_groups_fsc_only_iter(data, paramstructure, norm_per_particle, iteration):
    ...
    res_143 = Tracker["number_of_groups"] * [0]
    for index_of_colors in range(Blockdata["no_of_groups"]):
        group_start, group_end = MPI_volume_start_end(Tracker["number_of_groups"], Blockdata["no_of_groups"],
                                                      index_of_colors)
        if Blockdata["color"] == index_of_colors:  # It has to be 1 to avoid problem with tvol1 not closed on the disk
            for index_of_group in range(group_start, group_end):
                if Blockdata["myid_on_node"] == 0:
                    ...
                    tweight0 = get_im(
                        os.path.join(Tracker["directory"], "tempdir", "tweight_0_0_%d.hdf") % index_of_group)
                    tvol1 = get_im(os.path.join(Tracker["directory"], "tempdir", "tvol_1_0_%d.hdf") % index_of_group)
                    tweight1 = get_im(
                        os.path.join(Tracker["directory"], "tempdir", "tweight_1_0_%d.hdf") % index_of_group)
                    Util.fuse_low_freq(tvol0, tvol1, tweight0, tweight1, 2 * Tracker["fuse_freq"])
                    tag = 7007
                ...
                if Blockdata["myid_on_node"] == 1:
                    ...
                    tweight0 = get_im(
                        os.path.join(Tracker["directory"], "tempdir", "tweight_0_1_%d.hdf") % index_of_group)
                    tvol1 = get_im(os.path.join(Tracker["directory"], "tempdir", "tvol_1_1_%d.hdf") % index_of_group)
                    tweight1 = get_im(
                        os.path.join(Tracker["directory"], "tempdir", "tweight_1_1_%d.hdf") % index_of_group)
                    Util.fuse_low_freq(tvol0, tvol1, tweight0, tweight1, 2 * Tracker["fuse_freq"])
                    tag = 7007
                ...
                if Blockdata["myid_on_node"] == 0:
                    ...
                    cfsc = fsc(shrank0, shrank1)[1]
                    write_text_row(cfsc,
                                   os.path.join(Tracker["directory"], "fsc_driver_chunk0_grp%03d_iter%03d.txt") % (
                                   index_of_group, iteration))
                    del shrank0, shrank1
                    if (Tracker["nxinit"] < Tracker["constants"]["nnxo"]):
                        cfsc = cfsc[:Tracker["nxinit"]]
                ...
                if Blockdata["myid_on_node"] == 1:
                    ...
                    cfsc = fsc(shrank0, shrank1)[1]
                    write_text_row(cfsc,
                                   os.path.join(Tracker["directory"], "fsc_driver_chunk1_grp%03d_iter%03d.txt") % (
                                   index_of_group, iteration))
                    del shrank0, shrank1
                    if (Tracker["nxinit"] < Tracker["constants"]["nnxo"]):
                        cfsc = cfsc[:Tracker["nxinit"]]
...
def do3d_sorting_group_insertion_random_two_for_fsc(data, sparamstructure, snorm_per_particle):
    ...
    for index_of_groups in range(Tracker["number_of_groups"]):
        for procid in range(2):
            for ifsc in range(2):
                tvol, tweight, trol = recons3d_4nnsorting_group_fsc_MPI(myid=Blockdata["myid"],
                                                                        main_node=Blockdata["nodes"][procid], \
                                                                        prjlist=data, fsc_half=ifsc,
                                                                        random_subset=procid, group_ID=index_of_groups,
                ...
                if (Blockdata["myid"] == Blockdata["nodes"][procid]):
                    tvol.set_attr("is_complex", 0)
                    tvol.write_image(os.path.join(Tracker["directory"], "tempdir",
                                                  "tvol_%d_%d_%d.hdf" % (ifsc, procid, index_of_groups)))
                    tweight.write_image(os.path.join(Tracker["directory"], "tempdir",
                                                     "tweight_%d_%d_%d.hdf" % (ifsc, procid, index_of_groups)))
                    trol.write_image(os.path.join(Tracker["directory"], "tempdir",
...
def recons3d_4nnsorting_group_fsc_MPI(myid, main_node, prjlist, fsc_half, random_subset, group_ID, paramstructure,
                                      norm_per_particle, \
...
def compute_final_map(work_dir):
    ...
    number_of_groups = bcast_number_to_all(number_of_groups, Blockdata["main_node"], MPI_COMM_WORLD)
    Tracker = wrap_mpi_bcast(Tracker, Blockdata["main_node"], MPI_COMM_WORLD)
    if (number_of_groups == 0): ERROR("No clusters  found, the program terminates.", "compute_final_map", 1,
                                      Blockdata["myid"])
    compute_noise(Tracker["nxinit"])
    if (Blockdata["myid"] == Blockdata["main_node"]):
...
def do3d_sorting_groups_nofsc_final(rdata, parameterstructure, norm_per_particle):
    ...
    for index_of_groups in range(Tracker["number_of_groups"]):
        tvol, tweight, trol = recons3d_trl_struct_group_MPI(Blockdata["myid"], Blockdata["last_node"], rdata, 2,
                                                            index_of_groups, parameterstructure, norm_per_particle,
                                                            True, None, Tracker["constants"]["CTF"],
    ...
    keepgoing = bcast_number_to_all(keepgoing, source_node=Blockdata["main_node"],
                                    mpi_comm=MPI_COMM_WORLD)  # always check
    Tracker = wrap_mpi_bcast(Tracker, Blockdata["main_node"])
    if not keepgoing: ERROR("do3d_sorting_groups_nofsc_final  %s" % os.path.join(Tracker["directory"], "tempdir"),
                            "do3d_sorting_groups_nofsc_final", 1, Blockdata["myid"])
    return
    ...
    if nsgen > 0:
        if Blockdata["myid"] == Blockdata["main_node"]:
            ...
            log_file.add(
                '----------------------------------------------------------------------------------------------------------------')
            nclusters = 0
            log_file.add('{:^8} {:>8}   {:^24}  {:>15} {:^22} {:^5} {:^15} {:^20} '.format('Group ID', '    size',
            ...
            for ig1, value in list(Tracker["generation"].items()):
                ...
                for ic in range(value):
                    cluster_file = os.path.join(Tracker["constants"]["masterdir"], "generation_%03d" % ig,
                                                "Cluster_%03d.txt" % ic)
                    copyfile(cluster_file,
                             os.path.join(Tracker["constants"]["masterdir"], "Cluster_%03d.txt" % nclusters))
                    ...
                    cluster = read_text_file(os.path.join(Tracker["constants"]["masterdir"], "generation_%03d" % ig,
                                                          "Cluster_%03d.txt" % ic))
                    cluster_file = "Cluster_%03d.txt" % nclusters
                    vol_file = "vol_cluster%03d.hdf" % nclusters
                    msg = '{:>8} {:>8}   {:^24}        {:^6}          {:^6}          {:>5}  {:^15} {:^20} '.format(
                        nclusters, len(cluster), ig, round(all_gen_stat_list[ig][ic][0], 1),
                        round(all_gen_stat_list[ig][ic][1], 1), round(all_gen_stat_list[ig][ic][2], 1), cluster_file,
                        vol_file)
                    nclusters += 1
                    NACC += len(cluster)
                    log_file.add(msg)
            Unaccounted_file = os.path.join(Tracker["constants"]["masterdir"], "generation_%03d" % ig,
                                            "Unaccounted.txt")
            ...
            log_file.add('{:^7} {:^8} {:^22} {:^8} {:^24} {:^8} '.format(' Images', Tracker["constants"]["total_stack"],
                                                                         'accounted for images: ', NACC,
                                                                         'unaccounted for images: ', NUACC))
    ...
    for iloop in range(Nloop):
        ...
        for i in range(NT):
            ...
            for j in range(k):
                try:
                    clist[j].append(
                        float((np.intersect1d(new_clusters1[newindeces[j][0]], new_clusters2[newindeces[j][1]])).size)
                        / float(
                            (np.union1d(new_clusters1[newindeces[j][0]], new_clusters2[newindeces[j][1]])).size) * 100.)
                except:
                    pass
    ...
    while (keepsorting == 1) and (bad_clustering == 0):
        ...
        if keepchecking == 0:  # new, do it
            if Blockdata["myid"] == Blockdata["main_node"]:
                ...
                within_generation_restart = 0
            within_generation_restart = bcast_number_to_all(within_generation_restart, Blockdata["main_node"],
                                                            MPI_COMM_WORLD)
            if within_generation_restart == 1:
                read_tracker_mpi(work_dir)
            else:
                dump_tracker(work_dir)
            output_list, bad_clustering, stat_list = depth_clustering(work_dir, depth_order, my_pids, params,
                                                                      previous_params, log_main)
            all_gen_stat_list.append(stat_list)

            if bad_clustering != 1:
                if Blockdata["myid"] == Blockdata["main_node"]:
                    clusters, nclusters, nuacc = output_clusters(work_dir, output_list[0][0], output_list[0][1],
                ...
                if nclusters == 0:
                    ...
                    if Blockdata["myid"] == Blockdata["main_node"]:
                        ...
                        log_main.add('SORT3D generation%d time: %d hours %d minutes.' % (
...
def main():
    ...
    parser.add_option("--instack", type="string", default='',
    ...
    if initiate_from_meridien_mode:
        ...
        parser.add_option("--niter_for_sorting", type="int", default=-1,
                          help="User specified iteration number of 3D refinement for sorting")
        parser.add_option("--focus", type="string", default='',
                          help="Focus 3D mask. File path of a binary 3D mask for focused clustering ")
        parser.add_option("--mask3D", type="string", default='',
        ...
        parser.add_option("--nsmear", type="float", default=-1.,
        ...
        parser.add_option("--depth_order", type="int", default=2,
                          help="Depth order. A number defines the number of initial independent MGSKmeans runs (2^depth_order)")
        parser.add_option("--memory_per_node", type="float", default=-1.0,
                          help="Memory_per_node, the number used for computing the CPUs/NODE settings given by user")
        parser.add_option("--orientation_groups", type="int", default=100,
                          help="Number of orientation groups in the asymmetric unit")
        parser.add_option("--not_include_unaccounted", action="store_true", default=False,
                          help="Do not reconstruct unaccounted elements in each generation")
        parser.add_option("--stop_mgskmeans_percentage", type="float", default=10.0,
                          help="Swap ratio. A float number between 0.0 and 50")
        parser.add_option("--swap_ratio", type="float", default=1.0,
        ...
        parser.add_option("--do_swap_au", action="store_true", default=False,
                          help="Flag to turn on swapping the accounted for images with the unaccounted for images")
        parser.add_option("--random_group_elimination_threshold", type="float", default=2.0,
        ...
        if options.img_per_grp <= 1:
            ERROR("Improperiate input paramter for img_per_grp", "sort3d", 1, Blockdata["myid"])
        ...
        from sp_utilities import get_im, bcast_number_to_all, write_text_file, read_text_file, wrap_mpi_bcast, \
            get_params_proj, write_text_row
        from sp_filter import filt_tanl
        ...
        parser.add_option("--swap_ratio", type="float", default=1.0,
                          help="Randomness ratio of swapping accounted elements with unaccounted elemetns per cluster")
        parser.add_option("--notapplybckgnoise", action="store_true", default=False,
        ...
        if options.img_per_grp <= 1:
            ...
            ERROR("Parameter img_per_grp should be always larger than parameter minimum_grp_size", "sort3d", 1,
                  Blockdata["myid"])
