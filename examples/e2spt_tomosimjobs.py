...
def main():
	...
	if not options.plotonly:
		...
		if randstack and (options.testsampling or options.testboxsize or options.testlowpassfreq or options.testmaskradius):
			...
			nslices = ''
			if options.tiltstep:
				nslices = (old_div((2 * options.tiltrangelowerlimit), options.tiltstep) ) + 1						#You have to add 1 to account for the slice at 0deg
...
def simloop(options,rootpath,randstack,wildcard=''):
	...
	while tiltrange <tiltrangeu:
		...
		if options.tiltstep:
			...
			nslices = (old_div((2 * tiltrange), tiltstep) ) + 1						#You have to add 1 to account for the slice at 0deg
