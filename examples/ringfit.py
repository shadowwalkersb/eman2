...
def process_image(imgname,imgprefix):
	...
	for ttt in range(10):	# Run 10 times with random start poingt for best result (and ambiguous score? # todo #)
		...
		for i in range(200000):
			...
			if i%stepsz==0:
				...
				now_potential=pa.sim_potential()
				if(abs(old_div((now_potential-old_potential),old_potential))<.005):
					isstable+=1
					#print "aaa"
				else:
					isstable=0
	...
	print("%1.3g\t%1.3g\t%1.3g\t#"%(old_div(shape[2],shape[1]),old_div(shape[1],shape[0]),old_div(shape[2],shape[0])))
	return [old_div(shape[2],shape[1]),old_div(shape[1],shape[0]),old_div(shape[2],shape[0])]
