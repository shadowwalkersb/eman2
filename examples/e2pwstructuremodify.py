...
def distance_lines(p1,p2,q1,q2):
	...
	dist=old_div(abs(np.dot(c,crossab)),np.linalg.norm(crossab))
	return dist
...
def write_pdb(filename,ncent,SX,SY,SZ,color,atomnumber,vx):
	...
	for atom in range(shp[0]):
		...
		out.write(
			...
			%(atomnumber[atom], chain,atomnumber[atom] ,(ncent[atom,0]-old_div(SX,2))*vx[0], (ncent[atom,1]-old_div(SY,2))*vx[1], (ncent[atom,2]-old_div(SZ,2))*vx[2], 1, color[atom]) 
			)
		count+=1
	out.write("""TER  %6d      ALA %s%4d\n"""%(shp[0], chain, atom))
	out.write("END")
...
def calc_score(ncent,i,mrc,wd=1,wm=8,wa=2,ww=0,bl=0):
	...
	if have_alter==0: wd=old_div(wd,2)
	dis=dista+distb
	score=wd*dis+(old_div(wm,mrcp))+wa*ang+ww*distc
...
def main():
	...
	if crossbond==1:
		...
		for i in range(1,naa-1):
			pos1=old_div((ncent[i]+ncent[i+1]),2)
			mind=10
			for j in range(i+2,naa-1):
				pos2=old_div((ncent[j]+ncent[j+1]),2)
			...
			if (atomcolor[i]<options.thr):
				...
				for j in range(i+1,old_div((1+csbd[i]+i+1),2)):
					k=csbd[i]+i+1-j
