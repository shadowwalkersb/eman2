...
class EMMpiTaskHandler(object):
	...
	def check_task(self,id_list):
		...
		if chk[0]=="EROR":
			print("Error executing task {}".format(chk[1]))
			print(chk[2])
			print("Stop...")
			self.stop()
			return [-100 for i in id_list]
	...
	def test(self,verbose):
		...
		if self.rank==0:
			...
			while (1):
				...
				if com!="OK  " :
					print("MPI: Failed receive from node={} (com={})".format(src, com))
					mpi_finalize()
					sys.stderr.flush()
					sys.stdout.flush()
					os._exit(1)
			...
			a=mpi_bcast_recv(0)
			if a!="HELO" :
				print("MPI: Failed receive on node={} (rec={})".format(self.rank, a))
				mpi_finalize()
				sys.stderr.flush()
				sys.stdout.flush()
				os._exit(1)
			...
			while 1:
				...
				if select.select([self.mpifiler],[],[],0)[0]:
					...
					if com=="EXIT" :
						...
						for i in range(len(data)):
							...
							elif self.status[data[i]]==-100:
								## report error to handler
								data=("EROR", data[i], self.error)
								break
							else: 
								data[i]=self.status[data[i]]
				...
				if info:
					...
					if com=="DONE" :
						...
						print("Error in executing task {}".format(data[0]))
						print(data[1])
						self.error=data[1]
						self.status[data[0]]=-100
				...
				if r:
					...
					if com=="EXEC":
						...
						try: ret=task.execute(self.progress_callback)
							...
							err=traceback.format_exc()
							print(err)
							r=mpi_eman2_send("EROR",(self.task.taskid,err),0)
							break
...
class EMLocalTaskHandler(object):
	...
	lock=threading.Lock()
	allrunning = {}	# Static dict of running local tasks. Used for killing thses task upon parent kill
	def __init__(self,nthreads=2,scratchdir="/tmp", module=""):
		self.maxthreads=nthreads
