...
//	Py_INCREF(array_ptr);	//this is for letting EMData take the ownership of the data array
	...
	if(data_type == 'f') {
		double* array_data = (double*)PyArray_DATA(array_ptr);
		std::copy(array_data, array_data + (size_t)nx*ny*nz, temparray);
		image = new EMData((float*)temparray, nx, ny, nz);
	}
	else {
		PyArrayObject * array_ptr2 = (PyArrayObject*) PyArray_Cast(array_ptr, NPY_FLOAT32); //
		float* array_data2 = (float*)PyArray_DATA(array_ptr2);
		std::copy(array_data2, array_data2 + (size_t)nx*ny*nz, temparray);
		image = new EMData((float*)temparray, nx, ny, nz);
	}
