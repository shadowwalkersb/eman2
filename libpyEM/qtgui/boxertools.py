...
class Box(object):
	...
	def become(self,trimbox):
		...
		self.changed = trimbox.changed			# a flag signalling the box has changed and display needs updating
		self.TS = trimbox.TS
	...
	def change_box_size(self,box_size):
		...
		if box_size != self.xsize or box_size != self.ysize:
			self.xcorner -= old_div((box_size-self.xsize),2)
			self.xsize = box_size
			self.ycorner -= old_div((box_size-self.ysize),2)
			self.ysize = box_size
	...
	def get_small_box_image(self,autoboxer):
		...
		if image == None:
			...
			return image.get_clip(Region(int(old_div(self.xcorner,shrink)),int(old_div(self.ycorner,shrink)),int(old_div(self.xsize,shrink)),int(old_div(self.ysize,shrink))))
	...
	def center(self,method,extrasomething,low_res=False,update_image=True):
		...
		if method == Box.CENTEROFMASS:
			...
			if low_res == True:
				...
				ccf  = image.calc_ccf(None)
				trans = ccf.calc_max_location_wrap(-1,-1,-1)
				dx = old_div(trans[0],2)
				dy = old_div(trans[1],2)
...
class Cache(object):
	...
	be taken in regenerating the image
	...
	def get_image(self,image_name, *args, **kargs):
		...
		if image != None:
			# A segfault was occurring at this return statement
			#return image.copy()
			return image
		else:
...
class SincBlackmanSubsampledImage(object):
	'''
...
class CoarsenedFlattenedImage(object):
	def __init__(self,image_name):
		self.smallimage = None		# a small copy of an image which has had its background flattened
		self.image_name = image_name
...
class InverseSigmaImage(object):
	'''
	This is the inverse sigma image, as opposed to the sigma image, because in correlation the 
	image generated by the calc_ccf is divided by the sigma image. To save on unnecessary division,
...
class BinaryCircleImage(object):
...
class Boxable(object):
	...
	def __init__(self,image_name,parent,autoboxer=None):
		...
		# a list of deleted auto boxes - should be cleared every time an autobox occurs, and store interactively deleted boxes
		...
		self.template = None
		self.refcache = []
		self.template_ts = -1 # a template time stamp, used to avoid unnecessarily regenerating the template in self.auto_box
		self.autoboxer_state_ts = -1 # and autoboxer time stamp, used to avoid unnecessary autoboxing, and to force autoboxing when appropriate
	...
	def resize_moved_box_data(self,data,box_size,oldbox_size):
		adj = old_div((box_size-oldbox_size),2)
		for d in data:
			d[0] -= adj
			d[1] -= adj
			d[2] -= adj
	...
	def update_box_size(self,box_size):
		...
		for box in self.boxes:
			if box.xsize != box_size:
				box.xcorner -= old_div((box_size-box.xsize),2)
				box.xsize = box_size
				box.changed = True
			if box.ysize != box_size:
				box.ycorner -= old_div((box_size-box.ysize),2)
				box.ysize = box_size
				box.changed = True
...
class TrimSwarmTemplate(object):
	'''
	...
	def gen_template(self):
		...
		for n in range(0,4):
			...
			for idx,i in enumerate(images_copy):
				...
				a = image.get_clip(Region(int(old_div(box.xcorner,shrink)-dx),int(old_div(box.ycorner,shrink)-dy),size,size))
...
class ImageProcParamsMediator(object):
	'''
...
class TrimPawelAutoBoxer(object):
...
class PawelAutoBoxer(AutoBoxer):
	...
	def get_gaussh_param(self):
		...
		return old_div(ratio,self.box_size)
...
class TrimSwarmAutoBoxer(object):
	'''
